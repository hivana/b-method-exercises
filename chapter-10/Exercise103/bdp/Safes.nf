...
THEORY ListVariablesX IS
  ...
  Local_List_Variables(Machine(Safes))==(unlocks);
  List_Variables(Machine(Safes))==(unlocks,status,position,keys);
  External_List_Variables(Machine(Safes))==(unlocks,status,position,keys)
END
&
...
THEORY ListInvariantX IS
  ...
  List_Invariant(Machine(Safes))==(unlocks: KEY >+> DOOR & status~[{unlocked}] <: unlocks[keys] & keys <: dom(unlocks))
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Machine(Safes))==(position:=DOOR*{closed};status:=DOOR*{locked};keys:={};unlocks:={});
  Context_List_Initialisation(Machine(Safes))==(skip);
  List_Initialisation(Machine(Safes))==(unlocks:={})
END
&
...
THEORY ListOperationsX IS
  Internal_List_Operations(Machine(Safes))==(allocate,deallocate,insert,extract,unlock,quicklock,opendoor,lockdoor,closedoor);
  List_Operations(Machine(Safes))==(allocate,deallocate,insert,extract,unlock,quicklock,opendoor,lockdoor,closedoor)
END
&
...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Machine(Safes),allocate)==(kk: KEY & kk/:dom(unlocks) & dd: DOOR & dd/:ran(unlocks));
  List_Precondition(Machine(Safes),deallocate)==(kk: KEY & dd: DOOR & kk|->dd: unlocks & kk/:keys & status(dd) = locked);
  ...
  List_Precondition(Machine(Safes),quicklock)==(dd: DOOR & position(dd) = closed & dd: ran(unlocks))
END
&
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Machine(Safes),quicklock)==(dd: DOOR & position(dd) = closed & dd: ran(unlocks) & dd: DOOR & position(dd) = closed & unlocks~(dd): KEY | status:=status<+{dd|->locked} || keys:=keys-{unlocks~(dd)});
  Expanded_List_Substitution(Machine(Safes),unlock)==(dd: DOOR & unlocks~(dd): keys & dd: DOOR | status:=status<+{dd|->unlocked});
  ...
  Expanded_List_Substitution(Machine(Safes),deallocate)==(kk: KEY & dd: DOOR & kk|->dd: unlocks & kk/:keys & status(dd) = locked | unlocks:={kk}<|unlocks);
  Expanded_List_Substitution(Machine(Safes),allocate)==(kk: KEY & kk/:dom(unlocks) & dd: DOOR & dd/:ran(unlocks) | unlocks:=unlocks<+{kk|->dd});
  ...
  List_Substitution(Machine(Safes),allocate)==(unlocks(kk):=dd);
  List_Substitution(Machine(Safes),deallocate)==(unlocks:={kk}<|unlocks);
...
THEORY ListOfIdsX IS
  List_Of_Ids(Machine(Safes)) == (? | KEY,STATUS,locked,unlocked,DOOR,POSITION,open,closed | unlocks | V,keys,status,V,position | allocate,deallocate,insert,extract,unlock,quicklock | opendoor,closedoor,lockdoor | included(Machine(Locks)),included(Machine(Keys)) | ? | Safes);
...
THEORY VariablesEnvX IS
  Variables(Machine(Safes)) == (Type(status) == Mvl(SetOf(atype(DOOR,"[DOOR","]DOOR")*etype(STATUS,0,1)));Type(position) == Mvl(SetOf(atype(DOOR,"[DOOR","]DOOR")*etype(POSITION,0,1)));Type(keys) == Mvl(SetOf(atype(KEY,?,?)));Type(unlocks) == Mvl(SetOf(atype(KEY,?,?)*atype(DOOR,?,?))))
END
&
THEORY OperationsEnvX IS
  Operations(Machine(Safes)) == (Type(opendoor) == Cst(No_type,atype(DOOR,?,?));Type(closedoor) == Cst(No_type,atype(DOOR,?,?));Type(lockdoor) == Cst(No_type,atype(DOOR,?,?));Type(quicklock) == Cst(No_type,atype(DOOR,?,?));Type(unlock) == Cst(No_type,atype(DOOR,?,?));Type(extract) == Cst(No_type,atype(KEY,?,?)*atype(DOOR,?,?));Type(insert) == Cst(No_type,atype(KEY,?,?)*atype(DOOR,?,?));Type(deallocate) == Cst(No_type,atype(KEY,?,?)*atype(DOOR,?,?));Type(allocate) == Cst(No_type,atype(KEY,?,?)*atype(DOOR,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
