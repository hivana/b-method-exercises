...
THEORY ListPromotesX IS
  List_Promotes(Machine(Safes))==(opendoor,closedoor,lockdoor,quickclose)
END
&
...
THEORY ListVariablesX IS
  ...
  Local_List_Variables(Machine(Safes))==(?);
  List_Variables(Machine(Safes))==(status,position,keys);
  External_List_Variables(Machine(Safes))==(status,position,keys)
END
&
...
THEORY ListInvariantX IS
  ...
  List_Invariant(Machine(Safes))==(status~[{unlocked}] <: unlocks[keys])
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Machine(Safes))==(position:=DOOR*{closed};status:=DOOR*{locked};keys:={});
  Context_List_Initialisation(Machine(Safes))==(skip);
  List_Initialisation(Machine(Safes))==(skip)
END
&
...
THEORY ListOperationsX IS
  Internal_List_Operations(Machine(Safes))==(insert,extract,unlock,quicklock,quickcloseandlock,opendoor,lockdoor,quickclose,closedoor);
  List_Operations(Machine(Safes))==(insert,extract,unlock,quicklock,quickcloseandlock,opendoor,lockdoor,quickclose,closedoor)
END
&
THEORY ListInputX IS
  List_Input(Machine(Safes),closedoor)==(dd);
  List_Input(Machine(Safes),quickclose)==(dd);
...
THEORY ListOutputX IS
  ...
  List_Output(Machine(Safes),quickclose)==(?);
...
THEORY ListHeaderX IS
  ...
  List_Header(Machine(Safes),quickclose)==(quickclose(dd));
...
THEORY ListPreconditionX IS
  ...
  Own_Precondition(Machine(Safes),quickclose)==(dd: DOOR & position(dd) = open);
  List_Precondition(Machine(Safes),quickclose)==(dd: DOOR & position(dd) = open);
...
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Machine(Safes),quickcloseandlock)==(dd: DOOR & position(dd) = open & dd: DOOR & position(dd) = open & unlocks~(dd): KEY | position:=position<+{dd|->closed} || status:=status<+{dd|->locked} || keys:=keys-{unlocks~(dd)});
  ...
  List_Substitution(Machine(Safes),quickclose)==(closedoor(dd) || status(dd):=locked);
  Expanded_List_Substitution(Machine(Safes),quickclose)==(dd: DOOR & position(dd) = open | position:=position<+{dd|->closed} || status:=status<+{dd|->locked});
  ...
  List_Substitution(Machine(Safes),quickcloseandlock)==(quickclose(dd) || removekey(unlocks~(dd)))
END
&
...
THEORY ListANYVarX IS
  ...
  List_ANY_Var(Machine(Safes),quickclose)==(?);
...
THEORY ListOfIdsX IS
  List_Of_Ids(Machine(Safes)) == (unlocks | KEY,STATUS,locked,unlocked,DOOR,POSITION,open,closed | ? | V,keys,status,V,position | insert,extract,unlock,quicklock,quickcloseandlock | opendoor,closedoor,lockdoor,quickclose | included(Machine(Locks)),included(Machine(Keys)) | ? | Safes);
...
THEORY VariablesEnvX IS
  Variables(Machine(Safes)) == (Type(status) == Mvl(SetOf(atype(DOOR,"[DOOR","]DOOR")*etype(STATUS,0,1)));Type(position) == Mvl(SetOf(atype(DOOR,"[DOOR","]DOOR")*etype(POSITION,0,1)));Type(keys) == Mvl(SetOf(atype(KEY,?,?))))
END
&
THEORY OperationsEnvX IS
  Operations(Machine(Safes)) == (Type(opendoor) == Cst(No_type,atype(DOOR,?,?));Type(closedoor) == Cst(No_type,atype(DOOR,?,?));Type(lockdoor) == Cst(No_type,atype(DOOR,?,?));Type(quickclose) == Cst(No_type,atype(DOOR,?,?));Type(quickcloseandlock) == Cst(No_type,atype(DOOR,?,?));Type(quicklock) == Cst(No_type,atype(DOOR,?,?));Type(unlock) == Cst(No_type,atype(DOOR,?,?));Type(extract) == Cst(No_type,atype(KEY,?,?)*atype(DOOR,?,?));Type(insert) == Cst(No_type,atype(KEY,?,?)*atype(DOOR,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
